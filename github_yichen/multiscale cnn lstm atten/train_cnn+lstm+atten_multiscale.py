...
# After copying multiscale_cnn_lstm_attention.py contents into model.py, change to:
from model import MultiScale_CNN_LSTM_Attention
#from multiscale_cnn_lstm_attention import MultiScale_CNN_LSTM_Attention

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Multi-scale CNN-LSTM-Attention')
    ...
    parser.add_argument('--model_path',    type=str,   default='./results_multiscale_cnn_lstm_attention/')
    ...
    # cnn_channels: channels PER branch (total into LSTM = cnn_channels * 3)
    #               recommended: 64 — gives 192 total channels into LSTM
    #               lower than single-branch to keep model size comparable
    # cnn_stride  : compression factor, same as original CNN-LSTM-Attention
    #               recommended: 4
    # cnn_layers  : conv layers per branch, recommended: 2
    # ─────────────────────────────────────────────────────────────────────────
    parser.add_argument('--cnn_channels', type=int, default=64,
                        help='Channels per CNN branch (total = channels x 3)')
    parser.add_argument('--cnn_stride',   type=int, default=4)
    parser.add_argument('--cnn_layers',   type=int, default=2)

    # Range selection
    parser.add_argument('--range', type=str, default='250_900_1nm',
                        help='full | full_2nm | 250_900_1nm | 250_900_2nm | 400_850_1nm | 400_850_2nm')
    ...
    elif args.range == 'full_2nm':
        df_in_full = df.iloc[:, input_layers:]
        df_in     = df_in_full.iloc[:, ::2]   # 2nm resolution (every other column)
    elif args.range == '250_900_1nm':
        df_in = df[[f"R_{w}nm" for w in range(250, 901, 1)]]
    elif args.range == '250_900_2nm':
        df_in = df[[f"R_{w}nm" for w in range(250, 901, 2)]]
    elif args.range == '400_850_1nm':
        df_in = df[[f"R_{w}nm" for w in range(400, 851, 1)]]
    elif args.range == '400_850_2nm':
        df_in = df[[f"R_{w}nm" for w in range(400, 851, 2)]]
    else:
        raise ValueError(f"Unknown range: {args.range}")

    X = df_in.values.astype(float)
    Y = df_out.values.astype(float)
    print(f"Input shape: {X.shape}, Output shape: {Y.shape}")
    print(f"Multi-scale CNN branches: kernels [5, 11, 21], "
          f"channels per branch: {args.cnn_channels}, "
          f"total LSTM input: {args.cnn_channels * 3}")

    sc_in  = StandardScaler(); X_std = sc_in.fit_transform(X)
    sc_out = StandardScaler(); Y_std = sc_out.fit_transform(Y)

    # ── Output paths ─────────────────────────────────────────────────────────
    model_name = 'MultiScale_CNN_LSTM_Attention'
    model_path = os.path.join(
        args.model_path,
        f"{model_name}/hidden{args.hidden_size}_ch{args.cnn_channels}_stride{args.cnn_stride}_lr{args.lr}_{args.range}"
    )
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(X_std), 1):
        ...
        model = MultiScale_CNN_LSTM_Attention(
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch % args.val_step == 0:
                print(f"Fold {fold} | Epoch {epoch}/{args.epochs} | Train: {avg_train:.6f}")
                model.eval()
                with torch.no_grad():
                    total     = 0.0
                    preds_val = []
...
#    df_final.to_csv(f"{model_path}/pred.csv", index=False)
    df_final.to_csv(f"{model_path}/pred_{args.range}.csv", index=False)
